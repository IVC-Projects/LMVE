...
from LSVE_Single_ra_train import BATCH_SIZE
from LSVE_Single_ra_train import PATCH_SIZE
...
def get_test_list(HIGHDATA_Parent_PATH, lowDataList, labelList):
    doubleTest_list = []
    singleTest_list = []
    HighDirList = os.listdir(HIGHDATA_Parent_PATH)
    # convert string-list to int-list.
    HighDirList = list(map(int, HighDirList))
    ...
    for lowdata in lowDataList:
        ...
        if lowdataIndex % 4 != 0:
            if lowdataIndex in HighDirList:
                High_current_path = HIGHDATA_Parent_PATH + '/' + str(lowdataIndex)
                TWO_HighData = os.listdir(High_current_path)
                Two_HighData = [os.path.join(High_current_path, T) for T in TWO_HighData]
                tempData.append(Two_HighData)
...
#  return like this"[[hdata1, label1],[hdata2, label2]]" with the whole path.
def get_train_list(highDataList, labelList):
    assert len(highDataList) == len(labelList), \
        "high:%d, label:%d" % (len(highDataList), len(labelList))

    train_list = []
    for i in range(len(labelList)):
        tempData = []
        # this place should changed on the different situation.
        if int(os.path.basename(highDataList[i]).split('_')[-1].split('.')[0]) == \
                int(os.path.basename(labelList[i]).split('_')[-1].split('.')[0]):
            tempData.append(highDataList[i])
            tempData.append(labelList[i])

        else:
            raise Exception('len(lowData) not equal with len(highData)...')
        train_list.append(tempData)
    return train_list

# train_list like this"[[hdata1, label1],[hdata2, label2]]" with the whole path.
def prepare_nn_data(train_list):
    batchSizeRandomList = random.sample(range(0,len(train_list)), 8)
    gt_list = []
    highData_list = []
    for i in batchSizeRandomList:
        highData_image = c_getYdata(train_list[i][0])
        gt_image = c_getYdata(train_list[i][1])
        for j in range(0, 8):
        #crop images to the disired size.
            highData_imgY, gt_imgY = \
                crop(highData_image, gt_image, PATCH_SIZE[0], PATCH_SIZE[1], "ndarray")
            #normalize
            highData_imgY = normalize(highData_imgY)
            gt_imgY = normalize(gt_imgY)

            highData_list.append(highData_imgY)
            gt_list.append(gt_imgY)

    high1Data_list = np.resize(highData_list, (BATCH_SIZE, PATCH_SIZE[0], PATCH_SIZE[1], 1))
    gt_list = np.resize(gt_list, (BATCH_SIZE, PATCH_SIZE[0], PATCH_SIZE[1], 1))
    return high1Data_list, gt_list
...
def crop(highData_image, gt_image, patch_width, patch_height, img_type):
    assert type(highData_image) == type(gt_image), "types are different."
    highData_cropped = []
    gt_cropped = []

    # return a ndarray object
    if img_type == "ndarray":
        in_row_ind   = random.randint(0,highData_image.shape[0]-patch_width)
        in_col_ind   = random.randint(0,highData_image.shape[1]-patch_height)
        highData_cropped = highData_image[in_row_ind:in_row_ind+patch_width, in_col_ind:in_col_ind+patch_height]
        gt_cropped = gt_image[in_row_ind:in_row_ind+patch_width, in_col_ind:in_col_ind+patch_height]

    #return an "Image" object
    elif img_type == "Image":
        pass
    return highData_cropped, gt_cropped
